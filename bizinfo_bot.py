...
("사업자등록증명", ["사업자등록증명원", "사업자등록증명", "사업자등록증", "사업자등록"]),
    ...
    ("법인 등기부등본", ["법인등기부등본", "법인 등기부등본", "법인등기사항증명서", "말소사항 포함 법인등기부등본"]),
    ("법인세 신고내역", ["법인세신고내역", "법인세 신고내역", "법인세과세표준신고서"]),
    ("종합소득세 신고내역", ["종합소득세신고내역", "종합소득세 신고내역", "종합소득세과세표준신고서"]),
    ("부가세 신고내역", ["부가세신고내역", "부가가치세신고내역", "부가세 신고내역", "부가가치세 신고내역"]),
...
# "지원규모" 요약 문장에서 금액/건수를 각각 별도 컬럼으로 뽑아내기 위한 정규식.
# (전체 원문이 아니라 이미 요약된 짧은 '지원규모' 문장만 대상으로 하므로,
#  건수 단위 앞에 특정 동사가 붙어있지 않아도 안전하게 매칭할 수 있습니다.)
_COUNT_ONLY_RE = re.compile(r"(\d[\d,]*)\s*(건|개사|개\s*기업|개\s*업체|개소|개\s*팀|개|명)")


def extract_support_amount(budget_text):
    """'45백만원' -> '4500만원' 형태로 반환 (지원금액 컬럼용)."""
    won = parse_won_amount(budget_text)
    if won <= 0:
        return ""
    manwon = won // 10_000
    return f"{manwon}만원"
...
def extract_support_count(budget_text):
    """'28건 내외입니다' -> '28건' 형태로 반환 (지원갯수 컬럼용).
    원문 단위가 개사/명 등이어도 '건'으로 통일해서 표시합니다."""
    if not budget_text:
        return ""
    m = _COUNT_ONLY_RE.search(budget_text)
    if not m:
        return ""
...
def append_to_accumulator(items_data, path=ACCUMULATOR_PATH):
    ...
    if os.path.exists(path):
        try:
            ...
            link_idx = old_headers.index("원문링크") if "원문링크" in old_headers else None
    ...
    for row in items_data:
        ...
        if link and link in existing_links:
            continue
        new_rows.append([row.get(key, "") for _, key in EXPORT_COLUMNS])
        if link:
    ...
    wb_new.save(path)
    log(
        f"누적 파일 갱신: 기존 {len(existing_rows)}건 + 신규 {len(new_rows)}건 "
        f"= 총 {len(existing_rows) + len(new_rows)}건 ({path})"
    )
...
def main():
    ...
    def process_single_item(item):
        ...
        # 지원규모 문장(요약본)이 아니라, 원문에서 지원규모 관련 구절을 그대로
        # 뽑아온 relevant_context에서 직접 금액/건수를 파싱합니다. 요약 단계를
        # 거치면 "28건 내외입니다"처럼 특정 어미가 붙은 표현이 누락될 수 있어서,
        # 원문에 더 가까운 텍스트를 사용하는 것이 더 정확합니다.
        relevant_context = extract_relevant_context(body_text)
        support_source = relevant_context or body_text or budget
        support_amount = extract_support_amount(support_source)
        support_count = extract_support_count(support_source)
