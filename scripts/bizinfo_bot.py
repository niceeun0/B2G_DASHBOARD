...
BIZINFO_API_URL = (
    ...
    "?crtfcKey=Ofgt6R&dataType=json&searchCnt=100"
)
...
# =========================================================
DATE_FIELD_CANDIDATES = ["creatPnttm", "pubDate", "regDt", "creatPnttm1", "rceptDt"]
...
def deterministic_summary_hints(body_text):
    ...
    found_docs = [name for name in SPECIFIC_DOC_NAMES if _normalize_for_match(name) in normalized_body]
...
("사업자등록증명", ["사업자등록증명원", "사업자등록증명", "사업자등록증", "사업자등록", "사업자등록증(사본)"]),
    ("표준재무제표증명", ["표준재무제표증명원", "표준재무제표증명", "재무제표", "재무제표증명원", "재무제표증명서", "표준재무제표"]),
    ("부가가치세과세표준증명", ["부가가치세과세표준증명원", "부가가치세과세표준증명", "부가가치세 과세표준증명"]),
    ("납부내역증명", ["납부내역증명", "국세납부내역증명", "지방세납부내역증명", "국세 납부내역증명", "지방세 납부내역증명",]),
    ("납세증명", ["납세증명서", "납세증명", "국세납세증명서", "납세증명(국세)"]),
    ("면세사업자수입금액증명원", ["면세사업자수입금액증명원", "면세사업자수입금액증명", "면세사업자 수입금액증명"]),
    ("사업자등록증명(영문)", ["사업자등록증명원(영문)", "영문사업자등록증명", "사업자등록증명(영문)", "영문 사업자등록증명"]),
    ("4대보험완납증명서", ["4대보험완납증명서", "4대 보험 완납증명서", "4대보험 완납증명", "완납증명서(4대보험)"]),
    ("나의부동산정보", ["나의부동산정보", "부동산정보 통합열람", "나의 부동산 정보"]),
    ("지방세 납세증명", ["지방세납세증명서", "지방세 납세증명", "국세 및 지방세 완납증명서", "지방세완납증명서"]),
    ("지방세 세목별 과세증명", ["지방세세목별과세증명서", "지방세 세목별 과세증명", "세목별 과세증명서"]),
    ("법인 등기부등본", ["법인등기부등본", "법인 등기부등본", "법인등기사항증명서", "말소사항 포함 법인등기부등본", "등기부등본", "등기사항증명서"]),
    ("법인세 신고내역", ["법인세신고내역", "법인세 신고내역", "법인세과세표준신고서", "법인세신고서", "법인세 신고서"]),
    ("종합소득세 신고내역", ["종합소득세신고내역", "종합소득세 신고내역", "종합소득세과세표준신고서", "종합소득세신고서", "종합소득세 신고서"]),
    ("부가세 신고내역", ["부가세신고내역", "부가가치세신고내역", "부가세 신고내역", "부가가치세 신고내역", "부가세신고서", "부가가치세신고서"]),
    ("원천세 신고내역", ["원천세신고내역", "원천징수이행상황신고서", "원천세 신고내역", "원천징수 신고내역"]),
    ("거래처별합계표", ["거래처별세금계산서합계표", "거래처별합계표", "매출처별세금계산서합계표", "매입처별세금계산서합계표"]),
    ("산업재해율확인서", ["산업재해율확인서", "산재보험료율확인서", "산재율확인서"]),
    ("산업재해 요양승인 및 반려여부 확인서", ["산업재해요양승인", "요양승인 및 반려여부", "산업재해 요양승인", "요양승인반려여부확인서"]),
]
...
def _normalize_for_match(text):
    """
    '사업자 등록증'과 '사업자등록증'처럼 문서 텍스트 추출 과정에서 생기는
    공백 유무 차이를 무시하고 매칭하기 위해 모든 공백을 제거합니다.
    """
    if not text:
        return ""
...
def _get_docs_section_text(text):
    """
    '제출서류/구비서류' 등 섹션 근처 텍스트만 추출합니다. 서류 Y/N 판정을
    문서 전체가 아니라 이 섹션 안에서만 하면, "지원 제외 대상" 설명 등
    엉뚱한 곳에서 서류명이 언급되어 오탐되는 경우를 줄일 수 있습니다.
    섹션을 아예 못 찾으면(형식이 특이한 문서) 안전하게 전체 텍스트로
    폴백합니다 (정밀도보다 완전히 놓치는 것이 더 나쁘기 때문).
    """
    if not text:
        return ""
    windows = _find_keyword_windows(text, DOCS_KEYWORDS, before=0, after=1200, max_per_keyword=5)
    merged = _merge_windows(windows)
    if not merged:
        return text
...
def compute_oneclick_doc_flags(body_text):
    """카탈로그의 각 서류가 '제출서류 섹션' 근처에 언급되어 있는지 Y/N으로 판정.
    (공백 유무 차이는 무시하고 매칭)"""
    section_text = _normalize_for_match(_get_docs_section_text(body_text))
    flags = {}
    for label, keywords in ONECLICK_DOC_CATALOG:
        flags[label] = "Y" if (section_text and any(_normalize_for_match(kw) in section_text for kw in keywords)) else "N"
    return flags
...
def compute_etc_docs(body_text, max_items=8):
    ...
    etc = []
    for doc in doc_list:
        doc_norm = _normalize_for_match(doc)
        if any(_normalize_for_match(kw) in doc_norm for kw in _ALL_CATALOG_KEYWORDS):
            continue
        etc.append(doc)
...
_WON_AMOUNT_RE = re.compile(r"(\d[\d,]*(?:\.\d+)?)\s*(억|천만|백만|만)?\s*원")


def parse_won_amount(text):
    """
    텍스트에서 발견되는 금액 표현 중 가장 큰 값을 원(KRW) 단위 숫자로
    반환합니다 (우선순위 계산용, 근사치입니다). "1.85억원"처럼 소수점이
    붙은 표현도 정확히 처리합니다.
    """
    if not text:
        return 0
...
# "지원규모" 문장에서 금액/건수를 각각 별도 컬럼으로 뽑아내기 위한 정규식.
# (건, 개사, 명 뿐 아니라 "1기업당"처럼 '기업'만 붙는 표현도 인식)
_COUNT_ONLY_RE = re.compile(r"(\d[\d,]*)\s*(건|개사|개\s*기업|개\s*업체|개소|개\s*팀|개|명|기업)")


def extract_support_amount(text):
    """
    '1기업당 2,000원 이내' -> '2000원', '45백만원 이내' -> '45백만원' 형태로
    반환합니다. 원문에 실제로 쓰인 단위(원/만원/천만원/백만원/억원)를
    그대로 보존하고, 임의로 다른 단위로 환산하지 않습니다
    (환산이 필요한 숫자 계산은 parse_won_amount를 따로 사용).
    """
    if not text:
        return ""
    best_value = -1
    ...
    for m in _WON_AMOUNT_RE.finditer(text):
        try:
            ...
            continue
        unit = m.group(2) or ""
        value = num * _WON_UNIT_MULTIPLIER.get(unit, 1)
        if value > best_value:
            best_value = value
            best_num = num
            best_unit = unit
    if best_num is None:
        return ""
    num_str = str(int(best_num)) if best_num == int(best_num) else str(best_num)
    return f"{num_str}{best_unit}원"


def extract_support_count(text):
    """
    '28건 내외입니다' -> '28건', '1기업당 2,000원' -> '1개' 형태로 반환합니다.
    원문 단위가 '건'이면 '건'을 그대로 유지하고, 그 외(개사/기업/명 등)는
    일반적인 개수 표현인 '개'로 통일해서 표시합니다.
    """
    if not text:
        return ""
    ...
    unit = m.group(2)
    return f"{num}건" if unit == "건" else f"{num}개"


# =========================================================
# 지원규모 문장 원문 그대로 스크래핑 (AI 요약을 거치지 않음)
# =========================================================
def _split_sentences(text):
    """마침표/느낌표/물음표/줄바꿈 기준으로 문장을 대략 분리합니다."""
    if not text:
        return []
    parts = re.split(r"(?<=[.!?\n])\s*", text)
...
def _strip_thousand_commas(text):
    """'2,000원'처럼 숫자 사이의 천단위 콤마만 제거합니다 (문장 내 다른
    콤마, 예: '3개사, 최대 5천만원'의 콤마는 그대로 둡니다)."""
    return re.sub(r"(?<=\d),(?=\d)", "", text)


def extract_budget_sentence(body_text):
    """
    지원규모/사업비 등을 설명하는 문장을 AI 요약 없이 원문 그대로(천단위
    콤마만 제거) 뽑아옵니다. 여러 문장이 후보에 걸리면, 실제 금액이
    포함된 문장을 우선합니다. 못 찾으면 빈 문자열을 반환합니다.
    """
    if not body_text:
        return ""
    sentences = _split_sentences(body_text)
    keyword_hits = [s for s in sentences if any(kw in s for kw in BUDGET_KEYWORDS)]
    amount_hits = [s for s in sentences if _WON_AMOUNT_RE.search(s)]

    # 키워드와 금액이 둘 다 있는 문장을 최우선으로, 그다음 금액만 있는 문장,
    # 그다음 키워드만 있는 문장 순으로 폴백합니다.
    both = [s for s in keyword_hits if s in amount_hits]
    chosen = (both or amount_hits or keyword_hits or [None])[0]
    if not chosen:
        return ""
    return _strip_thousand_commas(chosen).strip()
...
_COLUMN_WIDTH_OVERRIDES = {
    ...
    "지원규모": 32, "지원금액": 12, "지원갯수": 10, "기타서류": 32, "문의처": 22, "담당이메일": 24,
...
# =========================================================
# 7-1. 누적 데이터 파일 갱신 (대시보드 자동화용)
# =========================================================
# 이 저장소의 data/daily_accumulated.xlsx에 오늘 수집분을 계속 쌓습니다.
# GitHub Actions 워크플로우가 이 파일을 매일 커밋해두면, build_dashboard.py가
# 항상 최신 누적본을 기준으로 대시보드를 재생성할 수 있습니다.
ACCUMULATOR_PATH = os.environ.get(
    "ACCUMULATOR_PATH",
    os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "data", "daily_accumulated.xlsx"),
)


def append_to_accumulator(items_data, path=ACCUMULATOR_PATH):
    """
    오늘 수집한 items_data를 영속 누적 파일에 추가합니다.
    '원문링크'를 기준으로 이미 누적되어 있는 공고는 다시 추가하지 않습니다
    (마감 전 공고가 다음날에도 계속 API에 남아있는 경우가 있어 중복 방지 필요).

    중요: 기존 행은 '몇 번째 칸'이 아니라 '컬럼 이름'을 기준으로 다시
    정렬해서 씁니다. EXPORT_COLUMNS 구조가 나중에 바뀌어도(컬럼 추가/삭제/
    순서변경) 예전에 쌓인 행들이 엉뚱한 컬럼으로 밀리지 않게 하기 위함입니다.
    (이 부분이 위치 기준으로 되어 있어서 실제로 컬럼이 밀리는 버그가 있었고,
    이번에 이름 기준 재정렬로 고쳤습니다.)
    """
    if not openpyxl:
        log("openpyxl 미설치 - 누적 파일 갱신을 건너뜁니다.")
        return
    ...
    if os.path.exists(path):
        try:
            wb_old = openpyxl.load_workbook(path)
            ws_old = wb_old.active
            first_row = next(ws_old.iter_rows(min_row=1, max_row=1), None)
            old_headers = [c.value for c in first_row] if first_row else []
            link_col = "원문링크"
            for raw_row in ws_old.iter_rows(min_row=2, values_only=True):
                # 위치가 아니라 '옛 헤더 이름 -> 값'으로 매핑해서 저장
                row_dict = dict(zip(old_headers, raw_row))
                existing_row_dicts.append(row_dict)
                link = row_dict.get(link_col)
                if link:
                    existing_links.add(link)

            added_cols = [h for h in headers if h not in old_headers]
            dropped_cols = [h for h in old_headers if h not in headers]
            if added_cols:
                log(f"스키마 변경 감지 - 새로 추가된 컬럼(기존 행은 빈칸으로 채움): {added_cols}")
            if dropped_cols:
                log(f"스키마 변경 감지 - 더 이상 쓰지 않는 컬럼(기존 행에서 버림): {dropped_cols}")
        except Exception as e:
            log(f"기존 누적 파일을 읽는 중 오류 - 새로 시작합니다: {e}")
    ...
    wb_new.save(path)
    log(
        f"누적 파일 갱신: 기존 {len(existing_row_dicts)}건 + 신규 {len(new_row_dicts)}건 "
        f"= 총 {len(existing_row_dicts) + len(new_row_dicts)}건 ({path})"
    )
...
def main():
    ...
    def _normalize_bizinfo_link(link):
        """상대경로('/web/lay1/...')로 오는 경우 도메인을 붙여 클릭 가능하게 만듭니다
        (공식 API 문서 예시에서도 도메인 없는 상대경로가 확인되어 추가한 안전장치)."""
        if not link:
            return ""
        if link.startswith("http"):
            return link
        return f"https://{BIZINFO_HOST}" + (link if link.startswith("/") else f"/{link}")
    ...
    def process_single_item(item):
        ...
        link = _normalize_bizinfo_link(item.get("pblancUrl") or item.get("rceptEngnHmpgUrl") or "")
        ...
        # 지원규모: AI 요약을 거치지 않고, 원문에서 지원규모를 설명하는
        # 문장을 그대로 스크래핑합니다 (천단위 콤마만 제거). 지원금액/
        # 지원갯수도 이 '같은 문장'에서 파싱해서 세 컬럼이 서로 어긋나지
        # 않고 항상 같은 원문 근거를 갖도록 합니다.
        budget_sentence = extract_budget_sentence(body_text)
        if not budget_sentence:
            # 문장 단위로 못 찾으면(문서 구조가 특이한 경우) AI 요약으로 폴백
            budget_sentence = summarize_budget(title, body_text)
        budget = budget_sentence
        support_amount = extract_support_amount(budget_sentence)
        support_count = extract_support_count(budget_sentence)
        ...
        relevant_context = extract_relevant_context(body_text)
        compare_text = relevant_context or body_text[:1500]
    ...
    def fallback_row(item, error):
        ...
        row = {
            ...
            "지원대상": "-",
            "기타서류": "",
            "원문링크": _normalize_bizinfo_link(item.get("pblancUrl") or item.get("rceptEngnHmpgUrl") or ""),
            "_compare_text": "",
            "_match_count": 0,
            "_budget_won": 0,
        }
    ...
    # 대시보드 자동화용: 누적 데이터 파일(data/daily_accumulated.xlsx) 갱신
    # (GitHub Actions 워크플로우가 이 파일을 커밋해서 build_dashboard.py가 사용)
    append_to_accumulator(items_data)
