...
def _write_df_sheet(wb, sheet_name, title, subtitle, df, col_widths=None):
...
def write_dashboard(df_rank, df_inst_raw, df_yearly_raw, df_daily_raw):
    ...
    rank_cols = [
        ...
        "사업규모점수", "우선순위_종합점수", "연락가능여부",
        "담당자_이메일_통합모음", "대표전화", "이메일",
    ]
    _write_df_sheet(
        wb, "대시보드_우선순위_랭킹",
        "B2G 솔루션 제안 우선순위 랭킹 대시보드 (전국 공공기관 현황 전수 반영)",
        "기준: 서류적합도 40% + 사업규모 35% + 활동성 25% 가중 백분위 (build_dashboard.py 참고)",
        df_rank[rank_cols],
        col_widths=[8, 22, 22, 20, 18, 14, 14, 12, 12, 14, 12, 60, 16, 24],
    )
